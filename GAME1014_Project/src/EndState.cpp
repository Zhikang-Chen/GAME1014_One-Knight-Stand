...
void EndState::Enter()
{
	...
	m_objects.push_back(make_pair("GameoverText", LP));	
	...
	m_objects.push_back(make_pair("SubText", ST));
	ST->SetPos((WIDTH - ST->GetDst()->w) / 2, HEIGHT / 3);

	// Adds an Image
	int width, height;
	SDL_QueryTexture(TEMA::GetTexture("NoIdea"), nullptr, nullptr, &width, &height);
	m_objects.push_back(make_pair("image", new SpriteObject({0,0,width, height }, {( WIDTH/2 ) - (float)width/4 ,HEIGHT / 2, (float)width/2,(float)height/2 }, TEMA::GetTexture("NoIdea"))));
