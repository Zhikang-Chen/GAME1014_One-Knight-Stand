...
void PauseState::Enter()
{
	...
	m_pbutton = new BoolButton({ 0,0,w/2,h }, { WIDTH / 2 - (float)w / 6,HEIGHT - 480,(float)w/2, (float)h }, TEMA::GetTexture("Back"));
	m_objects.emplace_back("button", m_pbutton);

	SDL_QueryTexture(TEMA::GetTexture("Exit"), nullptr, nullptr, &w, &h);
	m_pbutton2 = new BoolButton({ 0,0,w / 2,h }, { WIDTH / 2 - (float)w / 6, HEIGHT - 270,(float)w / 2, (float)h }, TEMA::GetTexture("Exit"));
	m_objects.emplace_back("button2", m_pbutton2);

	SDL_QueryTexture(TEMA::GetTexture("Save"), nullptr, nullptr, &w, &h);
	m_pbutton3 = new BoolButton({ 0,0,w / 2,h }, { WIDTH / 2 - (float)w / 6,HEIGHT-410,(float)w / 2, (float)h }, TEMA::GetTexture("Save"));
	m_objects.emplace_back("button3", m_pbutton3);

	SDL_QueryTexture(TEMA::GetTexture("Load"), nullptr, nullptr, &w, &h);
	m_pbutton4 = new BoolButton({ 0,0,w / 2,h }, { WIDTH / 2 - (float)w / 6, HEIGHT- 340,(float)w / 2, (float)h }, TEMA::GetTexture("Load"));
